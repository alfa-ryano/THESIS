...
This chapter presents the literature review of this study. Firstly, it presents some work on model persistence by highlighting some key characteristics -- unique features, strong points, and downsides -- of some existing tools. It then and summarises the advantages and drawbacks of state-based and change-based persistence and concludes what key characteristics for a new change-based persistence that can support a faster model differencing and conflict detection. Secondly, this chapter then reviews the related work on identifying differences and detecting conflicts between versions of models. It then presents the current challenges that model differencing and conflict detection are dealing with as well as the downsides of existing approaches in tackling the problems, which gives motivation to this research to come with a new solution. Thirdly, this chapter provides an overview of the research method employed in this research. Lastly, the conclusions of the literature review are presented. 
...
In constructing models, it is imperative that modelling tools can support model persistence so that under-construction models can be saved at any time and reloaded for further modification. Most of these tools persist models in their state-based format that is capturing a snapshot of a model at a time and then persist its entire state into a storage. The persistence can take different forms, such as text files, relational databases, or NoSQL databases.
...
The strong points of EMF Store are it was designed to allow semantic versioning of models, which can support model differencing and conflict detection to be more accurate and efficient rather than performing them on state-based model persistence. By default, the packages of operations are persisted in XMI files but it can also be configured to use other backends, e.g. MongoDB. The downsides of EMF Store are it has its own mechanism for controlling versions thus limits its adopters to use common text-oriented version controls, such as Git  and SVN. Its performance can also degrade as more models/users are added to a repository \cite{KolovosRMPGCLRV13}. EMF Store is discussed more in Section \ref{sec:a_new_change_based_persistence} on how its strong points and downsides affect the design of a new change-based persistence to enable high-performance model differencing and conflict detection.
...
integration with existing tools -- since it is still a non-standard approach -- for its adoption \cite{koegel2010EMF Store}, 
and still has limited support for standard, text-based version controls for collaborative development \cite{koegel2010EMF Store}. 
...
The summary of the advantages and downsides between change-based and state-based persistence are presented in Table \ref{table:advantages_drawbacks}.
    ...
    \begin{tabular}
      ...
      \begin{minipage}[t]{5cm}
        \begin{itemize}[leftmargin=9pt]
          ...
          \item[+] Faster and more accurate for comparison and merging \cite{DBLP:conf/sde/LippeO92,DBLP:conf/caise/IgnatN05,koegel2010EMF Store}
      ...
      \begin{minipage}[t]{5cm}
        ...
        \begin{itemize}[leftmargin=9pt]
          ...
          \item[+] A default standard, no need integration with existing tools \cite{koegel2010EMF Store}  
        \end{itemize}
      \end{minipage}
      \\
      \hline
      \multicolumn{2}{|p{2.2cm}|}{\centering Disadvantages} & \begin{minipage}[t]{5cm}
        ...
        \begin{itemize}[leftmargin=9pt]
          ...
          \item[--] Limited supports from standard, text-based version controls (e.g. GitHub) \cite{koegel2010EMF Store} 
          \item[--] Not a standard, need integration with existing tools \cite{koegel2010EMF Store} 
        \end{itemize}
      \end{minipage}
      & 
      \begin{minipage}[t]{5cm}
...
\subsection{Summary: Towards a New Change-based Model Persistence}
\label{sec:a_new_change_based_persistence}
The research introduced in this thesis aims at enabling high-performance model differencing and conflict detection which are the bottleneck when they are performed on models persisted in state-based format. This problem can be potentially solved by exploiting change-based model persistence. 

EMF Store exists as the only one implementation of change-based persistence for models. However, this research does not consider to extend EMF Store since it has some critical downsides that this study try to avoid: (1) This works aims for change-based persistence that can version and share models through different text-oriented version controls (e.g., Git, SVN) so that administration activities can be delegated to the version controls and to avoid dependency on EMF Store that uses its own mechanism to version models. (2) The performance of EMF Store can also degrade as more models/users are added to a repository \cite{KolovosRMPGCLRV13}.

This study also consider to adopt some strong points for the new change-based persistence. (1) Its implementation should be able to capture and persist relevant changes of models to allow accurate and efficient model differencing and conflict detection.
